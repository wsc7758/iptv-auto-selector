#!/usr/bin/env python3
"""
IPTV 自动优选 — 两阶段测速版（阶段一异步优化）
阶段一: 异步 HTTP 快速连通 + 速度测试，筛出每频道候选
阶段二: ffprobe 精测分辨率/码率/编码，按清晰度+速度综合排序
阶段三: ffmpeg 实际解码验证，检测黑屏/卡顿/串台
"""
import asyncio
import json
import os
...
async def fetch_text_limited(
    url: str, limit_bytes: int = 256 * 1024, session: ClientSession = None
) -> Tuple[bool, str, str, int, float, float]:
    """
    异步获取文本内容（m3u8 playlist），返回值和原版完全一致:
    ...
    try:
        async with session.get(
            url,
            headers=DEFAULT_HEADERS,
            timeout=ClientTimeout(connect=CONNECT_TIMEOUT, total=READ_TIMEOUT),
            ...
            async for chunk in resp.content.iter_chunked(32 * 1024):
                ...
                if time.perf_counter() - start >= SINGLE_URL_MAX_SECONDS:
                    break
            ok = 200 <= resp.status < 400 and total > 0
            text = b"".join(chunks).decode(resp.charset or "utf-8", errors="ignore")
...
async def test_direct_bytes(url: str, limit_bytes: int, session: ClientSession) -> dict:
    """
    异步测试直链流，返回结构和原版完全一致
    """
    start = time.perf_counter()
    ...
    try:
        async with session.get(
            ...
            if first_byte_time is not None:
                ...
                connect_time = now - start
                download_time = 0.0
            fmt = detect_content_format(content_type, first_chunk)
...
async def test_m3u8(url: str, session: ClientSession) -> dict:
    """
    异步测试 m3u8 直播源，返回结构和原版完全一致
    """
    result = {
        "ok": False,
        "bytes": 0,
    ...
    playlist_ok, text, content_type, playlist_bytes, conn_t, dl_t = await fetch_text_limited(url, session=session)
    ...
    if child_playlists and not segments:
        ...
        child_ok, child_text, child_type, child_bytes, child_conn, child_dl = await fetch_text_limited(child_url, session=session)
    ...
    for segment in segments[:M3U8_SEGMENT_TEST_COUNT]:
        seg_result = await test_direct_bytes(segment, SEGMENT_READ_BYTES, session)
...
def extract_service_name_from_hls(url: str) -> str:
    if not FFPROBE_PATH:
        return ""
    try:
        ok, text, _, _, _, _ = fetch_text_limited_sync(url, limit_bytes=128 * 1024)
        if not ok or "#EXTM3U" not in text:
            return ""
        variants = parse_m3u8_variants(url, text)
        if variants:
            child_url = variants[0]["url"]
            ok2, text2, _, _, _, _ = fetch_text_limited_sync(child_url, limit_bytes=128 * 1024)
...
# 同步兼容包装，供 ffprobe 阶段调用
def fetch_text_limited_sync(url: str, limit_bytes: int = 256 * 1024):
    import requests
    start = time.perf_counter()
    try:
        with requests.get(
...
async def test_single_url_async(url: str, url_keywords: List[str], history: dict, session: ClientSession) -> dict:
    """阶段一：异步 HTTP 快速测速，输出结构和原版完全一致"""
    ...
    if is_m3u8_url:
        test_result = await test_m3u8(url, session)
    else:
        test_result = await test_direct_bytes(url, DIRECT_READ_BYTES, session)
        if not test_result["ok"] and ("mpegurl" in test_result.get("content_type", "") or "m3u8" in test_result.get("content_type", "")):
            m3u8_result = await test_m3u8(url, session)
            if m3u8_result["ok"]:
...
def main() -> int:
    ...
    async def stage1_worker():
        nonlocal finished, timeout_count
        sem = asyncio.Semaphore(STREAM_TEST_WORKERS)
        async with ClientSession(
            timeout=ClientTimeout(total=SINGLE_URL_MAX_SECONDS + 5),
            headers=DEFAULT_HEADERS,
            connector=aiohttp.TCPConnector(ssl=False, limit=STREAM_TEST_WORKERS*2)
        ) as session:
            async def bounded_test(url):
                async with sem:
                    return await test_single_url_async(url, url_keywords, history, session)

            tasks = {asyncio.create_task(bounded_test(url)): url for url in unique_urls}
